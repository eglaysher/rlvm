...
Cp932::Cp932()
{
	DesirableCharset = SHIFTJIS_CHARSET;
	NoTransforms = true;
	UseUnicode = GetSystemDefaultLangID() != 0x411;
}

#ifndef NO_CP932_CONVERSION

const USHORT db_to_uni[] =
	{ 0x3000, 0x3001, 0x3002, 0xff0c, 0xff0e, 0x30fb, 0xff1a, 0xff1b, 
	  ...
	  0x9d6b, 0xfa2d, 0x9e19, 0x9ed1, 0xffff, 0xffff };

USHORT Cp932::Convert(USHORT ch) const
{
	if (ch < 0x7f)
		return ch;
	else if (ch >= 0xa1 && ch <= 0xdf)
		return ch + 0xfec0;
	else if (ch < 0x8140) // error, e.g. wrong character set selected
		return ch;
	else
		return db_to_uni[ch - 0x8140];
}

wchar_t* Cp932::ConvertString(const UCHAR* s) const
{
	wchar_t *rv = new wchar_t[strlen((char*) s) * 2 + 1];
	size_t i = 0;
	while (*s) {
		if (*s < 0x7f || *s >= 0xa1 && *s <= 0xdf)
			rv[i++] = Convert(*s++);
		else {
