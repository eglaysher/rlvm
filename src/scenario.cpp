...
{
	if (length < 0x1d0 || read_i32(data + 4) != 10002) throw Error("not a RealLive bytecode file");
	if (read_i32(data) != 0x1d0) throw Error("unsupported bytecode version");
	// Debug entrypoints
...
Script::Script(const Header& hdr, const char* data, const size_t length) : uptodate(true), strip(false)
{
	...
	Compression::decompress(data + read_i32(data + 0x20), read_i32(data + 0x28), uncompressed.get(), dlen);
	// Read bytecode
...
Scenario::rebuild()
{
	...
	size_t offset = 0;
	int line = 0;
	for (BytecodeList::const_iterator it = script.elts.begin(); it != script.elts.end(); ++it) {
	...
	for (int i = 1; i < 100; ++i) if (entrypoints[i] == -1) entrypoints[i] = entrypoints[i - 1];
	...
	for (std::vector<long>::const_iterator it = kidoku_table.begin(); it != kidoku_table.end(); ++it)
		append_i32(*rv, *it);
	if (!script.strip) for (std::vector<string>::const_iterator it = header.dramatis_personae.begin(); 
	...
	rv->append(header.rldev_metadata);	
...
is_goto_statement(const pointer_t& it)
{
	...
	return (elt.module() == 1 && elt.opcode() == 0) || (elt.module() == 5 && elt.opcode() == 1);
}
...
Scenario::optimise()
{
	...
	pointer_t it = begin();
	while (it != end()) {
		// If this is a line number immediately followed by another line number,
		// it's redundant; we can safely remove it.
		...
		if (it->type() >= Goto) {
			Pointers& ptrs = *it->get_pointers();
			for (Pointers::iterator pt = ptrs.begin(); pt != ptrs.end(); ++pt) {
				// If the pointer points to a "goto" statement, we can redirect it to the
				// destination of that jump, cutting out the middleman.
			...
			if (it->type() == Goto) {
				...
				do {
					++seek;
				         ...
				         && (seek->type() == Line || seek->type() == Kidoku || seek->type() == Entrypoint));
				if (seek == dest) {
					it = erase(it);
					continue;
				}
				else {
					// Conditional jumps that are always taken can be replaced with unconditional jumps.
					...
					// Likewise, conditional jumps that are never taken can be deleted.
					else if (taken == GotoElement::Never) {
						it = erase(it);
						continue;
					}
				}
				// Similarly, a goto to a non-returning statement can be replaced with that
				// statement directly.
				if (is_goto_statement(it) & does_not_return(dest)) {
					insert(next(it), dest->clone());
					// This is safe.  Outgoing pointers are erased automatically; incoming pointers 
					// are pushed onto the next element, i.e. the one we just inserted.
					it = erase(it);
				}
			}
		}
		
		// For unconditional jumps and returns, we can delete any following commands,
		// up to the next entrypoint or pointer destination, as dead code.
